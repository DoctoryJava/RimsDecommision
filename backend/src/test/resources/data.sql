-- H2 (MODE=MSSQLServer) 测试种子数据 —— 对齐 db/migration/sqlserver/V4__seed_r_data.sql（密码 demo1234）
-- 含中文的字面量统一加 N 前缀（T-SQL Unicode 字面量，H2 亦支持）
...
('pt-orders','orders',N'订单表','sys-001',N'[{"name":"order_id","type":"string","label":"订单编号"},{"name":"customer_id","type":"string","label":"客户编号"},{"name":"amount","type":"number","label":"金额"},{"name":"status","type":"select","label":"状态"}]',
...
('pt-customers','customers',N'客户表','sys-001',N'[{"name":"customer_id","type":"string","label":"客户编号"},{"name":"name","type":"string","label":"客户名称"}]',
N'[{"customer_id":"C001","name":"张伟","region":"华东"},{"customer_id":"C002","name":"李娜","region":"华北"}]'),
('pt-products','products',N'产品表','sys-001',N'[{"name":"product_id","type":"string","label":"产品编号"}]',
N'[{"product_id":"P100","product_name":"耳机"}]');
...
('qc-001',N'订单综合查询',N'关联订单客户产品','orders','[{"id":"j1","leftTable":"orders","leftColumn":"customer_id","rightTable":"customers","rightColumn":"customer_id","joinType":"left"}]',N'[{"alias":"order_id","table":"orders","column":"order_id","label":"订单编号"}]','{"field":"order_id","direction":"desc"}',10,'active','Sarah Chen','2026-06-01 00:00:00','2026-07-15 00:00:00'),
('qc-002',N'客户订单统计',N'按客户汇总','customers','[]',N'[{"alias":"name","table":"customers","column":"name","label":"客户名称"}]','{"field":"name","direction":"asc"}',10,'active','Robert Kim','2026-06-10 00:00:00','2026-07-20 00:00:00');
