    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button icon={<Save size={16} />} disabled={saving || tables.length === 0} onClick={saveAll}>
            {saving ? '保存中…' : '保存所有表'}
          </Button>
        </>
      }
      ...
      <div className="space-y-3">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* 全选 / 清空 + 已勾选计数（放在表格下方） */}
            <div className="flex items-center gap-3 text-xs border-t border-neutral-100 pt-3">
              <span className="text-neutral-500">只同步勾选的表：</span>
              <button onClick={() => setTables((prev) => prev.map((t) => ({ ...t, enabled: true })))} className="text-primary-600 hover:underline font-medium">全选</button>
              <span className="text-neutral-300">·</span>
              <button onClick={() => setTables((prev) => prev.map((t) => ({ ...t, enabled: false })))} className="text-neutral-500 hover:underline">取消全选</button>
